...
const getAllCategory = async (req, res) => {
  try {
    ...
    const categoryList = readyToParentAndChildrenCategory(categories, null, products);
    res.send(categoryList);
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const readyToParentAndChildrenCategory = (categories, parentId = null, products = []) => {
  const categoryList = [];
  let Categories;
  if (parentId == null) {
  ...
  for (let cate of Categories) {
    const productCount = products.filter(p => p.categories === cate._id.toString()).length;
    categoryList.push({
      _id: cate._id,
      name: cate.name,
      ...
      children: readyToParentAndChildrenCategory(categories, cate._id, products),
    });
  }
