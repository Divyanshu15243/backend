...
} = require("../lib/referral/referralHelper");
...
const registerCustomer = async (req, res) => {
  ...
  try {
    const { name, email, password, referralCode } = jwt.decode(token);
  ...
  try {
    ...
    const usersWithOwnerProfit = await Promise.all(
      users.map(async (user) => {
        ...
        const ownerProfit = orders.reduce(
          (sum, order) => sum + (order.ownerProfit || 0),
          0
        );
        return {
          ...user.toObject(),
...
const addShippingAddress = async (req, res) => {
  try {
    ...
    if (result.matchedCount > 0) {
      return res.send({ message: "Shipping address added or updated successfully." });
    } else {
